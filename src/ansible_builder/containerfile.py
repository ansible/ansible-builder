...
class Containerfile:
    ...
    def _insert_global_args(self, include_values: bool = False):
        ...
        global_args = {
            ...
            "ANSIBLE_GALAXY_CLI_COLLECTION_OPTS": self.definition.build_arg_defaults[
                "ANSIBLE_GALAXY_CLI_COLLECTION_OPTS"
            ],
            "ANSIBLE_GALAXY_CLI_ROLE_OPTS": self.definition.build_arg_defaults[
        ...
        for arg, value in global_args.items():
            ...
            if (
                include_values
            ):  # emit `ARG` directives for empty strings so they can be overridden at build-time
    ...
    def _prepare_galaxy_install_steps(self):
        ...
        if self.galaxy_required_valid_signature_count:
            install_opts += (
                f" --required-valid-signature-count {self.galaxy_required_valid_signature_count}"
            )
    ...
    def _prepare_introspect_assemble_steps(self):
        ...
        if any(self.definition.get_dep_abs_path(thing) for thing in ("galaxy", "system", "python")):
            ...
            requirements_file_exists = os.path.exists(
                os.path.join(
