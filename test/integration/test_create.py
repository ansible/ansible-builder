...
def test_definition_syntax_error(cli, data_dir):
    ...
    assert r.rc != 0
    assert "An error occurred while parsing the definition file" in (r.stdout + r.stderr), (
        r.stdout + r.stderr
    )
...
def test_create_streams_output_with_verbosity_on(cli, build_dir_and_ee_yml):
    ...
    assert (
        "Ansible Builder is generating your execution environment build context." in result.stdout
    )
...
def test_inline_str_galaxy_requirements(cli, build_dir_and_ee_yml):
    ...
    assert yaml.safe_load(req_out_content) == {
