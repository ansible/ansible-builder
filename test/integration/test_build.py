...
def test_user_python_requirement(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    for py_library in "requirements-parser":
        result = cli(
            f"{runtime} run --rm {ee_tag} /usr/libexec/platform-python -m pip show {py_library}",
            allow_error=True,
        )
...
def test_build_layer_reuse(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    test_index = [
        idx for idx, value in enumerate(out_lines) if 'RUN echo "$(echo hi) $(echo mom)"' in value
    ][0]
...
def test_collection_verification_on(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    result = cli(
        f"ansible-builder build --no-cache --galaxy-keyring {keyring} -c {tmp_path} -f {ee_def} -t {ee_tag} --container-runtime {runtime} -v 3",
        allow_error=True,
    )
...
def test_galaxy_signing_extra_args(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    result = cli(
        ...
        f"--galaxy-required-valid-signature-count 3",
        allow_error=True,
    )
